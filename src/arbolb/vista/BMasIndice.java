...
public class BMasIndice extends VistaArbol{
	private static final long serialVersionUID = 1L;

	public BMasIndice(ArbolB arbol){
		super(arbol);
		setTipo("B+ Índice");
	}
	...
	public String toGraph(String accion) {
		...
		if(arbol.getCantElementos() != 0){
			...
			str += "<TR><TD ALIGN=\"TEXT\" HEIGHT=\"20\" WIDTH=\"40\" BGCOLOR=\"" + VistaArbol.COLOR_PUNT_BMAS + "\"><B> Puntero a Siguiente Hoja </B></TD></TR> \n";
			if (accion != null){
			...
			for(int i = 1; i < hojas.size(); i++){
				str += hojas.get(i-1).getId() + " -> " + hojas.get(i).getId() + " [color=blue] \n";
			}
